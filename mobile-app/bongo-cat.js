// Bongo cat scene + animation.
// Original artwork/animation: CodePen by @StrayRogue and @DitzyFlama
// (https://codepen.io/abeatrize/pen/LJqYey). Converted here from
// Pug/SCSS/TypeScript to plain HTML/CSS/JS, and adapted so the animation
// starts/stops with real typing state (instead of looping forever) and so
// multiple independent copies (You / Partner) can run on the same page.
//
// Requires GSAP + DrawSVGPlugin, loaded via <script> tags before this file
// (see index.html). Both are free as of GSAP's 2025 Webflow-sponsored
// license change -- no club membership needed.

let __bongoInstanceCounter = 0;

const BONGO_CAT_SCENE_HTML = `
  <svg class="bongo-cat-scene" id="{{ID}}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 783.55 354.91">
    <g class="bongo-cat">
      <g class="head">
      ...
      <!-- Cosmetic accessories for unlockable skins. Hidden by default;
           style.css shows the right one via .bongo-cat-scene[data-skin="..."].
           Positions are a best-effort estimate against this abstract/geometric
           art style (it has no literal face), nudge the coordinates in
           style.css/bongo-cat.js if they look off once you see them live. -->
      <g class="accessory accessory-partyhat">
        <polygon points="493,26 537,26 515,-18"></polygon>
        <circle cx="515" cy="-20" r="7"></circle>
      </g>
      <g class="accessory accessory-bandana">
        <polygon points="376,202 428,202 402,234"></polygon>
      </g>
    </g>
  </svg>
`;

function mountBongoCat(container, options = {}) {
  const id = `bongo-cat-${++__bongoInstanceCounter}`;
  container.innerHTML = BONGO_CAT_SCENE_HTML.replace(/\{\{ID\}\}/g, id);

  const svgEl = container.querySelector('svg.bongo-cat-scene');
  svgEl.setAttribute('data-skin', options.skin || 'classic');
  const s = (selector) => `#${id} ${selector}`;

  if (typeof gsap === 'undefined') {
    // GSAP failed to load (offline, blocked CDN, etc). Fail quietly with a
    // static cat rather than throwing.
    return {
      setTyping() {},
      celebrate() {},
      setSkin(skinId) {
        svgEl.setAttribute('data-skin', skinId || 'classic');
      },
      react() {},
      setDuet() {},
      syncPawPhase() {},
  ...
  // Triplicate each note (like the original) so three can fly at once.
  ...
  const paw = {
    leftUp: s('.paw-left .up'),
    leftDown: s('.paw-left .down'),
    rightUp: s('.paw-right .up'),
    rightDown: s('.paw-right .down'),
  };

  gsap.set(s('.music .note'), { scale: 0, autoAlpha: 0 });
  gsap.set([paw.leftUp, paw.rightUp], { autoAlpha: 0 });
  gsap.set([paw.leftDown, paw.rightDown], { autoAlpha: 1 });

  const pawFlicker = (selector) =>
    gsap.fromTo(
      selector,
      { autoAlpha: 0 },
  ...
  const pawTl = gsap.timeline({ paused: true });
  pawTl
    .add(pawFlicker(paw.leftUp), 'start')
    .add(pawFlicker(paw.rightDown), 'start')
    .add(pawFlicker(paw.leftDown), 'start+=0.19')
    .add(pawFlicker(paw.rightUp), 'start+=0.19')
    .timeScale(1.6);

  const hasDrawSVG = typeof DrawSVGPlugin !== 'undefined';
  const codeTween = hasDrawSVG
    ? gsap.fromTo(
        s('.terminal-code line'),
        { drawSVG: '0%' },
        { drawSVG: '100%', duration: 0.1, stagger: 0.1, ease: 'none', repeat: -1, paused: true }
      )
    : null;

  const colorizer = gsap.utils.random(
    ['#a5ea9b', '#ff61d8', '#569cfa', '#ffcc81', '#7ed1e2', '#a3a4ec', '#67b5c0', '#fd7c6e'],
    true
  );
  const rotator = gsap.utils.random(-50, 50, 1, true);
  const dir = (amt) => `${gsap.utils.random(['-', '+'])}=${amt}`;

  const allNotes = gsap.utils.shuffle(gsap.utils.toArray(s('.music .note')));
  const per = Math.max(1, Math.floor(allNotes.length / 3));
  const noteGroups = [allNotes.slice(0, per), allNotes.slice(per, per * 2), allNotes.slice(per * 2)];
  let notesActive = false;

  // Typing speed -> animation speed. `keysPerSec` comes from the caller
  // (measured from real keystroke/input timing). We clamp it to a sane
  // range and map it onto a GSAP timeScale, so hammering the keyboard makes
  // the paws/code/notes visibly speed up, and slow, thoughtful typing keeps
  // things calmer.
  const BASE_TIME_SCALE = 1.6;
  const MIN_KPS = 1; // keystrokes/sec at or below this -> slowest animation
  const MAX_KPS = 12; // keystrokes/sec at or above this -> fastest animation
  const MIN_SCALE = 0.8;
  const MAX_SCALE = 3.2;
  let currentTimeScale = BASE_TIME_SCALE;
  let wasTyping = false;

  function speedToTimeScale(keysPerSec) {
    if (!keysPerSec || keysPerSec <= 0) return BASE_TIME_SCALE;
    const clamped = Math.min(MAX_KPS, Math.max(MIN_KPS, keysPerSec));
    const t = (clamped - MIN_KPS) / (MAX_KPS - MIN_KPS);
    return MIN_SCALE + t * (MAX_SCALE - MIN_SCALE);
  }

  function flyNotes(els) {
    if (!notesActive || !els.length) return;
    els.forEach((el) => {
  ...
  function resetIdlePose() {
    gsap.set([paw.leftUp, paw.rightUp], { autoAlpha: 0 });
    gsap.set([paw.leftDown, paw.rightDown], { autoAlpha: 1 });
    if (hasDrawSVG) gsap.set(s('.terminal-code line'), { drawSVG: '0%' });
    gsap.set(s('.music .note'), { autoAlpha: 0 });
  }

  return {
    // isTyping: boolean. keysPerSec (optional): recent typing rate, used to
    // scale how fast the paws/code/notes animate.
    setTyping(isTyping, keysPerSec) {
      svgEl.classList.toggle('typing', !!isTyping);
      svgEl.classList.toggle('idle', !isTyping);
      if (isTyping) {
        currentTimeScale = speedToTimeScale(keysPerSec);
        pawTl.timeScale(currentTimeScale);
        if (codeTween) codeTween.timeScale(currentTimeScale);
        if (!wasTyping) {
          // Only restart from frame 0 when transitioning idle -> typing;
          // while already typing we just retune the speed in place so the
          // motion doesn't stutter on every keystroke.
          pawTl.play(0);
          if (codeTween) codeTween.play(0);
        }
        if (!notesActive) {
          notesActive = true;
          noteGroups.forEach((group, i) => setTimeout(() => flyNotes(group), i * 150));
        }
        wasTyping = true;
      } else {
        pawTl.pause(0);
        if (codeTween) codeTween.pause(0);
        notesActive = false;
        wasTyping = false;
        currentTimeScale = BASE_TIME_SCALE;
        resetIdlePose();
      }
    },
    celebrate() {
      svgEl.classList.add('celebrate');
      setTimeout(() => svgEl.classList.remove('celebrate'), 1500);
    },
    // One-off reactions for events other than the continuous typing state --
    // 'error' (something broke), 'terminal' (a terminal/editor opened), or
    // 'poke' (your partner nudged you). Wire these up to whatever signal you
    // like (see main.js's webhook).
    react(type) {
      const durations = { error: 900, terminal: 700, poke: 650 };
      const ms = durations[type];
      if (!ms) return;
      Object.keys(durations).forEach((t) => svgEl.classList.remove(`reaction-${t}`));
      svgEl.classList.add(`reaction-${type}`);
      setTimeout(() => svgEl.classList.remove(`reaction-${type}`), ms);
    },
    // Swap the color palette / accessories (see style.css skin rules).
    setSkin(skinId) {
      svgEl.setAttribute('data-skin', skinId || 'classic');
    },
    // Toggles the "we're both typing right now" glow/highlight.
    setDuet(isDuet) {
      svgEl.classList.toggle('duet', !!isDuet);
    },
    // Restarts the paw-bounce loop from frame 0 -- call on both cats at the
    // same moment so they visually bounce in sync when a duet starts.
    syncPawPhase() {
      if (wasTyping) pawTl.play(0);
    },
  };
}

if (typeof window !== 'undefined') {
  window.mountBongoCat = mountBongoCat;
}
